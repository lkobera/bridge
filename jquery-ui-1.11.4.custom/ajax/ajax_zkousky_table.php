<?
	...
	$sql='	SELECT
			zkousky_terminy.*, SUM(zkousky_zaci.PPV) AS usedPPV, SUM(zkousky_zaci.PJ_A) AS usedPJ_A, SUM(zkousky_zaci.PJ_B) AS usedPJ_B
...
<? while ($radek=mysqli_fetch_array($result)):?>
  <tr class="<? if ($radek['locked']==1) echo 'active'?>">
	  ...
	  <td><? echo $radek['cas'].'<br>'.$radek['venue_ID']?></td>
		  <td><!--Capacity Badge-->
		  <span class="badge 
