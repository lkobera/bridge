...
<div class="container-fluid">
	<div class="page-header">
		<h1><small><?php echo $_SESSION['loc']?> | Admin | Registrace žáků</small><br />Editace studenta</h1>
    ...
    <form class="form" method="post" action="index.php?menu=registrace&request=<? echo $_GET['request']?>">
    ...
    <div class="well">
		    ...
		    <div class="row form-group">
			    <div class="col-sm-4">
			    	<div class="panel panel-default">
			            <div class="panel panel-heading"><strong>Provozovna</strong></div>
			            <div class="panel panel-body"><select id="loc" name="loc" class="form-control" type="text"></select></div>
					</div>
				</div>
        	</div>
		<? endif?>
        ...
        <a href="?menu=registrace&request=<? echo $_GET['request']?>" class="btn btn-primary"><span class="glyphicon glyphicon-remove"></span>&nbsp;Zrušit změny</a>
	</div>
...
<script>
    $.getJSON('ajax/branch.json', function(json) {
        $.each(json, function() {
			if (this.loc=='<?=$radek['loc']?>') $('#loc').append('<option selected value="'+ this.loc +'">' + this.city + '</option>');
			else $('#loc').append('<option value="'+ this.loc +'">' + this.city + '</option>');
        });
    });
</script>